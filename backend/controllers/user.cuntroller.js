...
import generatedAccessToken from '../utils/generatedAcceToken.js'
...
export async function verifyEmailController(request,responce) {
    try{
        ...
        const updateUser = await UserModel.updatrOne ({_id : code},
            {
                verify_email : true
            }
        )
...
export async function loginController(request,responce) {
    try {
        ...
        return responce.status.status(500).json({
            message : error.message || error,
            error : true,
            success : false
        })
...
export async  function uploadAvatar(request,response){
    try {
        const userId = request.userId //auth middl ware
...
export async function forgotPasswordController(request,response) {
    try {
        ...
        await sendEmail({
            sendTo : email,
            subject : "Forgot password from online shop",
